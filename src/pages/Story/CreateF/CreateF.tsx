...
export const CreateF = () => {
    ...
    if (act) {
      try {
        const res = await apis.post(
          "/books/builder/dalle-init",
          bookInitData,
          config
        );
        console.log(res.data);
        setBg1(res.data.data[0].bookInsight.imageUrl);
  ...
  return (
    <S.Container>
        ...
        <>
          ...
          {isCreated ? (
            ...
            <>
              ...
              {isClicked && (
                <>
                  <TTS text={`${nameAtomValue}는 ${bgInit}에서 무엇을 하고 있어?`} speaker="nwoof" />
                  <BubbleG
                    text={`${nameAtomValue}(은/는) ${bgInit}에서 무엇을 하고 있어?`}
                    length={52}
                  />
      ...
      {isDict && <Dictionary isOpen={true} closeModal={dictClose} context={text1} />}
    </S.Container>
  );
};
